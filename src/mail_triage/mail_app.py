...
class AppleScriptMail:
    ...
    def _send_script(self, to_address: str, subject: str, body: str) -> str:
        ...
        **Every send leaves a copy in Drafts, and this script cannot prevent
        it.** Measured twice on 6 August 2026: sent 19:48:56, draft 19:49:25;
        sent 20:25:15, draft 20:25:25. Appending ``delete newMessage`` after
        the send was tried and made no difference — Mail has already written
        the autosaved draft to the server by then, and deleting the compose
        object does not retract it. The draft is left alone rather than hunted
        down in the Drafts mailbox and deleted: deleting stored mail on a
        match of subject and recipient is a far worse failure mode than a
        stray draft. Tidying up is a decision for the caller, not for the one
        method that sends.
        """
        return (
            'tell application "Mail"\n'
